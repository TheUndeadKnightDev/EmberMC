...
FlintMC tracks one Paper commit, named in `gradle.properties`:

```properties
paperCommit = a2a42c5b12249aaba42a347327fd930a1f94af06
```

Everything FlintMC changes is a patch on top of that commit. Updating means
...
5. If it fails, paperweight stops at the first patch that no longer applies
   ...
   ```bash
   cd paper-server            # or paper-api, or flint-server/src/minecraft/java
   git status                 # see the conflicted files
   # edit, keeping every "// Flint" line intact
...
- Prefer FlintMC's own classes under `flint-*/src` over patching Paper. A
  one-line hook in an upstream file plus a whole class of our own conflicts far
  less than a large diff in the upstream file.
- Keep every changed upstream line marked `// Flint - <reason>` and every block
  wrapped in `// Flint start - <reason>` / `// Flint end - <reason>`. Conflicts
