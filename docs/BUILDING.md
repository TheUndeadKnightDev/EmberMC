...
3. **A seeded upstream cache.** paperweight fetches Paper into
   ...
   `scripts/env.sh` clones Paper into that directory (no checkout) whenever it
   is missing, so paperweight always finds a repository where it expects one.
   Never delete the root `.gradle` directory by hand. A `GIT_CEILING_DIRECTORIES`
   fence is *not* a substitute: `rebuild*Patches` legitimately walks up from the
   patch directories to stage regenerated patches into this repository, and a
   ceiling breaks that.

`scripts/env.sh` does all three. Source it before building. If the working
